...
; #VARIABLE# ====================================================================================================================
Global $__g_sReportTitle_Debug = "AutoIt 调试报告"
...
Func _Assert($sCondition, $bExit = True, $iCode = 0x7FFFFFFF, $sLine = @ScriptLineNumber, Const $_iCurrentError = @error, Const $_iCurrentExtended = @extended)
	...
	If Not $bCondition Then
		MsgBox($MB_SYSTEMMODAL, "AutoIt 调试断言", "调试断言失败 (行 " & $sLine & "): " & @CRLF & @CRLF & $sCondition)
...
Func _DebugBugReportEnv(Const $_iCurrentError = @error, Const $_iCurrentExtended = @extended)
	...
	If @OSLang <> StringRight(@KBLayout, 4) Then $sKBLayout = ", 键盘: " & @KBLayout
	If @OSArch <> @CPUArch Then $sCPUArch = ", CPUArch: " & @CPUArch
	Return SetError($_iCurrentError, $_iCurrentExtended, "AutoIt 版本: " & @AutoItVersion & $sAutoItX64 & $sAdminMode & $sCompiled & _
			", 操作系统: " & @OSVersion & $sOsServicePack & "/" & @OSArch & _
			", 操作系统语言: " & @OSLang & $sMUIlang & $sKBLayout & $sCPUArch & _
			", 脚本: " & @ScriptFullPath)
	...
	If IsInt($_iCurrentError) Then
		...
		If UBound($sTemp) > 1 Then
			...
			Switch $_iCurrentError
				Case 0
					$sData &= "不良返回 " & $sTemp[1] & " 在 " & $sTemp[0] & ".dll"
				Case 1
					$sData &= "无法打开 " & $sTemp[0] & ".dll"
				Case 3
					$sData &= "找不到 " & $sTemp[1] & " 在 " & $sTemp[0] & ".dll"
			EndSwitch
		EndIf
	EndIf
...
Func _DebugReportVar($sVarName, $vVar, $bErrExt = False, Const $iDebugLineNumber = @ScriptLineNumber, Const $_iCurrentError = @error, Const $_iCurrentExtended = @extended)
	...
	Local $sData = "@@ 调试(" & $iDebugLineNumber & ") : " & __Debug_DataType($vVar) & " -> " & $sVarName
...
Func __Debug_ReportClose()
	If $__g_iReportType_Debug = 1 Then
		...
		_DebugReport('>>>>>> 请关闭<报告日志窗口>后退出 <<<<<<<' & @CRLF)
		__Debug_ReportWindowWaitClose()
