...
Func _GUICtrlEdit_Find($hWnd, $bReplace = False)
	...
	Local $hGuiSearch = GUICreate("查找", 349, 177, -1, -1, BitOR($__UDFGUICONSTANT_WS_CHILD, $__EDITCONSTANT_WS_MINIMIZEBOX, $__EDITCONSTANT_WS_CAPTION, $__EDITCONSTANT_WS_POPUP, $__EDITCONSTANT_WS_SYSMENU))
	Local $idStatusBar1 = _GUICtrlStatusBar_Create($hGuiSearch, $aPartsRightEdge)
	_GUICtrlStatusBar_SetText($idStatusBar1, "查找: ")

	GUISetIcon(@SystemDir & "\shell32.dll", 22, $hGuiSearch)
	GUICtrlCreateLabel("查找:", 9, 10, 53, 16, $__EDITCONSTANT_SS_CENTER)
	Local $idInputSearch = GUICtrlCreateInput("", 80, 8, 257, 21)
	Local $idLblReplace = GUICtrlCreateLabel("替换:", 9, 42, 69, 17, $__EDITCONSTANT_SS_CENTER)
	Local $idInputReplace = GUICtrlCreateInput("", 80, 40, 257, 21)
	Local $idChkWholeOnly = GUICtrlCreateCheckbox("完全匹配", 9, 72, 145, 17)
	Local $idChkMatchCase = GUICtrlCreateCheckbox("区分大小写", 9, 96, 145, 17)
	Local $idBtnFindNext = GUICtrlCreateButton("查找下一个", 168, 72, 161, 21, 0)
	Local $idBtnReplace = GUICtrlCreateButton("替换", 168, 96, 161, 21, 0)
	Local $idBtnClose = GUICtrlCreateButton("关闭", 104, 130, 161, 21, 0)
	If (IsArray($aSel) And $aSel <> $EC_ERR) Then
		...
		If $aSel[0] <> $aSel[1] Then ; text was selected when function was invoked
			...
			WEnd
		EndIf
		_GUICtrlStatusBar_SetText($idStatusBar1, "查找: " & GUICtrlRead($idInputSearch))
	EndIf
	...
	Else
		_GUICtrlStatusBar_SetText($idStatusBar1, "替换数: " & $iReplacements, 1)
		_GUICtrlStatusBar_SetText($idStatusBar1, "结果: ", 2)
	EndIf
	GUISetState(@SW_SHOW)
		...
		Select
			...
			Case $iMsgFind = $idBtnFindNext
				...
				If $bReplace = True Then
					_GUICtrlStatusBar_SetText($idStatusBar1, "查找: " & GUICtrlRead($idInputSearch))
					_GUICtrlStatusBar_SetText($idStatusBar1, "结果: " & GUICtrlRead($idInputReplace), 2)
				EndIf
...
Func __GUICtrlEdit_FindText($hWnd, $idInputSearch, $idChkMatchCase, $idChkWholeOnly, ByRef $iPos, ByRef $iOccurance, ByRef $iReplacements)
	...
	If $sFind <> "" Then
		...
		ElseIf $iWhole And Not $iPos Then ; no more to find
			$iOccurance = 0
			MsgBox($MB_SYSTEMMODAL, "查找", "已到文件尾端, 不能继续查找'" & $sFind & "'")
