...
;                  |  $BOM_DEFPARADIR       - Default paragraph direction梚mplies alignment (obsolete).
...
;                  |Obsolete bits are valid only for the bidirectional version of Rich Edit 1.0.
;                  |  $BOE_RTLDIR           - Default paragraph direction梚mplies alignment (obsolete).
...
Func _GUICtrlRichEdit_GetParaSpacing($hWnd)
	...
	Switch $iRule
		...
		Case 5
			$sRet = StringFormat("%.2f", $iInter / 20) & " 行;"
	EndSwitch
