...
Func _viFindGpib(ByRef $aDescriptorList, ByRef $aIDNList, $iShow_Search_Results = 0)
	...
	If $i_Num_instr < 1 Then ; No insturments were found
		If $iShow_Search_Results = 1 Then
			MsgBox($MB_SYSTEMMODAL, "GPIB 搜索结果", "未发现 GPIB 总线设备")
		EndIf
	...
	If $iShow_Search_Results = 1 Then
		...
		Next
		MsgBox($MB_SYSTEMMODAL, "GPIB 搜索结果", $s_Search_results)
	EndIf
...
Func _viInteractiveControl($sCommand_Save_FilePath = "")
	...
	While 1
		...
		$s_Vi_id = InputBox("仪器设备描述符", _
				"- 输入仪器设备描述符 (例如 'GPIB::1::0' 或 'GPIB::1::INSTR')" & _
				@CR & @CR & _
				"- 输入查找对象执行 GPIB 搜索" & _
				@CR & @CR & _
				"- 单击 [取消] 停止 VISA 交互工具", $s_Vi_id, "", 500, 250)
		If @error = 1 Then
		...
		If IsString($s_Answer) Then
			...
			MsgBox($MB_SYSTEMMODAL, "查询结果", "[" & $s_Vi_id & "] " & $sCommand & " -> " & $s_Answer)
		ElseIf $s_Answer = 0 Then
			;- The command was not a query but it was exuced successfully
			MsgBox($MB_SYSTEMMODAL, "命令结果", "命令:" & @CR & @CR & _
					"         '" & $sCommand & "'" & @CR & @CR & _
					"在设备上执行成功: " & @CR & @CR & _
					"         '" & $s_Vi_id & "'")
		ElseIf $s_Answer < 0 Then
			;- There was an error -> Show an error message
			$s_Answer = MsgBox($MB_SYSTEMMODAL, "VISA 错误", _
					"执行 VISA 命令时发生错误:" & @CR & @CR & _
					"'" & $sCommand & "'" & @CR & @CR & "在设备 '" & $s_Vi_id & "'" & _
					@CR & @CR & _
					"要在继续之前将 GPIB 总线重置吗?")
			If $s_Answer = 6 Then ; Yes
				_viGpibBusReset()
				MsgBox($MB_SYSTEMMODAL, "VISA", "GPIB 总线已重置!")
			EndIf
		EndIf
	WEnd
	...
	If $sCommand_list <> $s_Empty_command_list Then
		...
		If $sCommand_Save_FilePath = "" Then
			...
			$s_Answer = MsgBox(64 + 4, "将命令保存到 AutoIt3 脚本吗?", _
					"要保存你发布的命令到 AutoIt3 脚本吗?")
			If $s_Answer = 6 Then ; Yes
				$sCommand_Save_FilePath = FileSaveDialog("另存为...", @ScriptDir, _
						"AutoIt3 脚本 (*.au3)", 16, "visa_log.au3")
				If @error Then
