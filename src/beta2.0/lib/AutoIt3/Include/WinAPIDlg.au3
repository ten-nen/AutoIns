	...
	If Not @error Then
		Switch $aResult[0]
			Case $CDERR_DIALOGFAILURE
				Return SetError($aResult[0], 0, "无法创建对话框." & @LF & _
						"常见对话框函数调用 DialogBox 会话框函数失败." & @LF & _
						"例如, 如果通用对话框调用时指定一个无效的窗口句柄.")
			Case $CDERR_FINDRESFAILURE
				Return SetError($aResult[0], 0, "常见对话框函数未能找到指定的资源.")
			Case $CDERR_INITIALIZATION
				Return SetError($aResult[0], 0, "常见对话框函数初始化时失败." & @LF & "当没有足够的可用内存时通常会出现此错误.")
			Case $CDERR_LOADRESFAILURE
				Return SetError($aResult[0], 0, "常见对话框函数不能加载指定的资源.")
			Case $CDERR_LOADSTRFAILURE
				Return SetError($aResult[0], 0, "常见对话框函数不能加载指定的字符串.")
			Case $CDERR_LOCKRESFAILURE
				Return SetError($aResult[0], 0, "常见对话框函数不能锁定指定的资源.")
			Case $CDERR_MEMALLOCFAILURE
				Return SetError($aResult[0], 0, "常见对话框函数不能为内部结构分配内存.")
			Case $CDERR_MEMLOCKFAILURE
				Return SetError($aResult[0], 0, "常见对话框函数无法锁定与句柄关联的内存.")
			Case $CDERR_NOHINSTANCE
				Return SetError($aResult[0], 0, "ENABLETEMPLATE 标志在对应通用对话框初始化结构的 Flags 成员中设置," & @LF & _
						"但你没有提供相应的实例句柄.")
			Case $CDERR_NOHOOK
				Return SetError($aResult[0], 0, "ENABLEHOOK 标志在对应通用对话框初始化结构的 Flags 成员中设置," & @LF & _
						"但你没有提供一个指向相应钩子程序的指针.")
			Case $CDERR_NOTEMPLATE
				Return SetError($aResult[0], 0, "ENABLETEMPLATE 标志在对应通用对话框初始化结构的 Flags 成员中设置," & @LF & _
						"但你没有提供相应的模板.")
			Case $CDERR_REGISTERMSGFAIL
				Return SetError($aResult[0], 0, "通用对话框函数调用时 RegisterWindowMessage 函数返回错误代码.")
			Case $CDERR_STRUCTSIZE
				Return SetError($aResult[0], 0, "对应通用对话框初始化结构的 lStructSize 成员无效")
			Case $FNERR_BUFFERTOOSMALL
				Return SetError($aResult[0], 0, "用户文件名指定 OPENFILENAME 结构的 lpstrFile 成员指向的缓冲区太小." & @LF & _
						"lpstrFile 指定缓冲区的前两个字节包含一个指定大小的 TCHAR 整数值, 这是接收完整名称所需的.")
			Case $FNERR_INVALIDFILENAME
				Return SetError($aResult[0], 0, "文件名称无效.")
			Case $FNERR_SUBCLASSFAILURE
				Return SetError($aResult[0], 0, "子类化一个列表框的操作失败, 因没有足够的内存.")
		EndSwitch
	EndIf
